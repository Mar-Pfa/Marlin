...
void calibrate_print_surface(float z_offset) {
    ...
    if (abs(y) >= 3)
      {
		bed_leveltmp[1][y + 3] = bed_leveltmp[2][y + 3];
		bed_leveltmp[5][y + 3] = bed_leveltmp[4][y + 3];
      }
    if (abs(y) >=2) 
      {
		bed_leveltmp[0][y + 3] = bed_leveltmp[1][y + 3];
		bed_leveltmp[6][y + 3] = bed_leveltmp[5][y + 3];
      }
    // Print calibration results for manual frame adjustment.
    for (int x = -3; x <= 3; x++) 
      {
		SERIAL_PROTOCOL_F(bed_leveltmp[x + 3][y + 3], 3);
      SERIAL_PROTOCOLPGM(" ");
      }
    SERIAL_ECHOLN("");
  }
...
// copy tempdata to new bed_level_data
	for (int y = -3; y <= 3; y++)
	{
		for (int x = -3; x <= 3; x ++)
		{
			bed_level[x + 3][y + 3] = bed_leveltmp[x + 3][y + 3];
		}
	}
}
/*
...
float probe_bed(float x, float y)
  ...
  {
	  SERIAL_PROTOCOL_F(probe_bed_array[i], 5);
	  SERIAL_ECHO(" ");
  }
  SERIAL_ECHOLN("");
...
void process_commands()
{
  ...
  if(code_seen('G'))
  {
    ...
    case 29: // G29 Calibrate print surface with automatic Z probe.
	  ...
	  if (code_seen('S'))
	  {
		  ...
		  if (code_seen('Z'))
			  kz = code_value();
		  if (kx < 0 || kx>6 || ky<0 || ky>6)
		  {
			  SERIAL_ECHOLN("invalid value for x or y (only 0 to 6 allowed)");
			  break;
		  }
		  bed_level[kx][ky] = kz;
		  break;
	  ...
	  SERIAL_ECHOLN("G29 B for Bedleveling");
	  SERIAL_ECHOLN("G29 C for Clear Level Data");
	  SERIAL_ECHOLN("G29 D for Display Level Data");
	  SERIAL_ECHOLN("G29 I for Invert Level Data");
	  SERIAL_ECHOLN("G29 S X Y Z for Set Level Point");
  ...
  else if(code_seen('M'))
  {
	...
	case 305: // M305
	{
		SERIAL_PROTOCOLLN("Enable/Disable Software Min Endstop");
		if (code_seen('E')) {
			SERIAL_PROTOCOLLN("Enabled!");
			min_software_endstops = true;
			break;
		}
		if (code_seen('D')) {
			SERIAL_PROTOCOLLN("Disabled!");
			min_software_endstops = false;
			break;
		}
		SERIAL_PROTOCOLLN("E for enable, D for Disable");
		SERIAL_PROTOCOLLN("currently: ");
    ...
    case 302: // allow cold extrudes, or set the minimum extrude temperature
    {
	  ...
	  SERIAL_PROTOCOLLN("set min temp for extrude:");
	  SERIAL_ECHO(temp);
    }
    break;
	#endif
